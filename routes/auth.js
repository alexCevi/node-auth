...
router.post('/resetPassword', async (req, res) => {
    ...
    const user = await User.findOne({email: req.body.email, resetPasswordExpDate: {$gt: Date.now()}})
    if(!user) return res.status(400).send({
        error: "Expired reset code" // can assume this because user has already provided email
    });
    if(req.body.resetCode !== user.resetPasswordCode) return res.status(400).send({
